...
export default function ProductionReadinessPage() {
  ...
  const hasAccess = session && (isAdminUser(user) || hasPermission(user, "settings.productionReadiness.view"));
  ...
  const runCheck = async (checkKey?: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(checkKey ? `Executed check: ${checkKey} successfully.` : "Executed all system diagnostics.");
      } else {
