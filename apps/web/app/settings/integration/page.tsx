...
export default function SapIntegrationHub() {
  ...
  const hasAccess = session && (isAdminUser(user) || hasPermission(user, "settings.integration.view"));
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-xs font-bold text-on-surface-variant">Loading Integration Hub...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-status-error/10 border border-status-error/20 rounded-xl p-8 max-w-lg mx-auto text-center mt-12">
        ...
        <p className="text-sm text-on-surface-variant mt-2">
          You do not have the required permissions to access the Integration Hub.
        </p>
      </div>
    );
  }
