...
export default function MasterDataHubPage() {
  ...
  const hasAccess = session && (isAdminUser(user) || hasPermission(user, "masterdata.view"));
    ...
    return (
      <div className="flex h-96 items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-xs font-bold text-on-surface-variant">Loading Master Data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex h-96 items-center justify-center">
        <div className="text-center max-w-md p-6 bg-status-error/10 border border-status-error/20 rounded-2xl">
          ...
          <p className="text-xs text-on-surface-variant mt-1">
            You must possess administrative credentials to access Master Data Hub.
          </p>
        </div>
      </div>
    );
