...
export default function IncidentsPlaceholder() {
  ...
  return (
    <div className="flex-1 bg-[#F9F9FF] p-8 font-['IBM_Plex_Sans',_sans-serif] min-h-[85vh]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-[#E7EEFF] pb-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold text-[#001A48] tracking-tight">Incident Review Center</h1>
          ...
          <p className="text-sm text-[#444651] max-w-xl">
            Review security hazard reports, fence breaches, and facility issues submitted by field officers.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#C4C6D2] rounded-lg p-12 text-center mb-8 shadow-sm">
        <span className="material-[#002D72] material-symbols-outlined text-4xl mb-4">folder_open</span>
        <h3 className="text-base font-bold text-[#001A48] mb-1">No Incidents Reported</h3>
        <p className="text-xs text-[#747782] max-w-sm mx-auto mb-6">
          Report logs, severity levels, status tracking, and photo attachments will show up inside this center.
        </p>
        <button disabled className="px-4 py-2 bg-[#002D72]/50 text-white text-xs font-bold rounded cursor-not-allowed">
          Create Incident Log (Phase 1B)
        </button>
      </div>
      ...
      <div className="bg-white border border-[#C4C6D2] rounded-lg p-6 shadow-sm">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-[#444651] list-inside list-disc">
          <li><strong>Photo & Media Evidence:</strong> Gallery reviewer of uploaded inspection cameras.</li>
          <li><strong>Workflow Escalation:</strong> Assign incident status to managers, operations, or client contacts.</li>
          <li><strong>Roster Integration:</strong> Auto-links incident reporter to active duty schedules.</li>
        </ul>
      </div>
    </div>
  );
