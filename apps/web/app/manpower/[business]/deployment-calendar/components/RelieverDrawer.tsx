...
export const RelieverDrawer: React.FC<RelieverDrawerProps> = ({
  ...
  const fetchEligibleEmployees = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/manpower/scheduling/eligible-employees?slotId=${slot.id}`);
  ...
  if (!isOpen || !slot || !exception) return null;
          ...
          <header className="p-6 border-b border-outline-variant bg-surface-container-low flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-secondary mt-1">
                Replacing <span className="font-semibold">{primaryAssignment?.employee?.name}</span> for {slot.snapshotPosition} on {new Date(slot.businessDate).toISOString().split("T")[0]}.
              </p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            {loading ? (
              ...
              filteredEmployees.map((item) => {
                const isFullyEligible = item.canDeploy && item.warnings.length === 0;
                const hasWarnings = item.canDeploy && item.warnings.length > 0;
                const isBlocked = !item.canDeploy;
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-bold text-foreground text-sm flex items-center gap-2">
                          {item.employee.name}
                          <span className="text-xs text-secondary font-mono">({item.employee.id})</span>
                        </h4>
