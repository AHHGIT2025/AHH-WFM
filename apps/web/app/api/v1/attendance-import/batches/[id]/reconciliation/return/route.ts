...
import { isAttendanceReconciliationEnabled, returnReconciliation } from "@/lib/attendance-reconciliation-engine";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await returnReconciliation(recBatch.id, returnReason, {
    ...
    return NextResponse.json({ error: err.message || "Failed to return reconciliation batch." }, { status: 400 });
  }
}