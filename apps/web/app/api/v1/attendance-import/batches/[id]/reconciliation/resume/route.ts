...
import { isAttendanceReconciliationEnabled, resumeReturnedReview } from "@/lib/attendance-reconciliation-engine";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await resumeReturnedReview(recBatch.id, {
    ...
    return NextResponse.json({ error: err.message || "Failed to resume review." }, { status: 400 });
  }
}