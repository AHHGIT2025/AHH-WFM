...
import { generateDetailedTimesheetWorkbook } from "@/lib/attendance-import-exporter";
...
export async function GET(
  ...
  try {
    ...
    const excelBuffer = await generateDetailedTimesheetWorkbook(batch.id);

    return new Response(excelBuffer as any, {
      status: 200,
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="Detailed_Timesheet_DRAFT_${batch.batchNumber}.xlsx"`
      }
    });
  } catch (error: any) {
    console.error("Failed to generate detailed timesheet export:", error);
    return NextResponse.json({ error: error.message || "Failed to generate detailed timesheet export." }, { status: 500 });
  }
}
