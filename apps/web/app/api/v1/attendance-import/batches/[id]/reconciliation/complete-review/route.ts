...
import { isAttendanceReconciliationEnabled, completeReview } from "@/lib/attendance-reconciliation-engine";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await completeReview(recBatch.id, {
    ...
    return NextResponse.json({ error: err.message || "Failed to complete review." }, { status: 400 });
  }
}