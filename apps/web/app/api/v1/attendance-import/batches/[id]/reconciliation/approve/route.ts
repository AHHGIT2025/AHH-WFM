...
import { isAttendanceReconciliationEnabled, approveReconciliation } from "@/lib/attendance-reconciliation-engine";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await approveReconciliation(recBatch.id, {
      id: user.id || "SYS_ACTOR",
      name: user.name || "Approver",
      role: user.role
    });

    return NextResponse.json({ success: true, reconciliationBatch: result.batch, snapshot: result.snapshot });
  } catch (err: any) {
    return NextResponse.json({ error: err.message || "Failed to approve reconciliation." }, { status: 400 });
  }
}