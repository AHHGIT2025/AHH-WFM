...
import { isAttendanceReconciliationEnabled, reopenReconciliation } from "@/lib/attendance-reconciliation-engine";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await reopenReconciliation(recBatch.id, reopenReason, {
    ...
    return NextResponse.json({ error: err.message || "Failed to reopen reconciliation batch." }, { status: 400 });
  }
}