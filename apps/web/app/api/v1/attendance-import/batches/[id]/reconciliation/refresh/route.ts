...
import { isAttendanceReconciliationEnabled, refreshEvidence } from "@/lib/attendance-reconciliation-engine";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await refreshEvidence(recBatch.id, {
    ...
    return NextResponse.json({ error: err.message || "Failed to refresh evidence." }, { status: 400 });
  }
}