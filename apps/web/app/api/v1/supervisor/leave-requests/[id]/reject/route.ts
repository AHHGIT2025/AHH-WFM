...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!isAuthorized && (auth.session.user as any).role !== "ADMIN") {
      return NextResponse.json({ error: "Unauthorized to reject this leave request" }, { status: 403 });
    }

    const updated = await mockDb.updateLeaveStatus(params.id, "Rejected");
    if (!updated) {
      return NextResponse.json({ error: "Leave request not found" }, { status: 404 });
    }

    return NextResponse.json({ message: "Leave rejected successfully", leave: updated });
  } catch (e) {
