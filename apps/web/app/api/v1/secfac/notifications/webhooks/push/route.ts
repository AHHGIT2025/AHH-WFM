...
export async function POST(req: NextRequest) {
  ...
  const expectedSecret = process.env.SECFAC_PUSH_WEBHOOK_SECRET;

  if (process.env.SECFAC_PUSH_ENABLED !== "true") {
    return NextResponse.json({ error: "Push provider is disabled." }, { status: 403 });
  }
  ...
  try {
    ...
    await prisma.secFacNotificationAttempt.update({
      ...
      data: {
        status: mappedStatus,
        responseMessage: `Push webhook callback: ${mappedStatus}`,
    ...
    console.error("POST /api/v1/secfac/notifications/webhooks/push error:", e);
